...
reference_numbers <- "author_ref_numbers.xlsx" # stores the manuscript reference numbers associated with each study 
# Parameters
...
include_vigour_baselineRTnonsign <- 1 # 1 (default) if including all vigour studies - including the studies with data only for high reward but baseline is not significant, otherwise 0
...
# Add column with the correct reference number in manuscript to use for forest plots (JAMA psychiatry requirement)
datAll <- match_ref("author_ref_numbers.xlsx", datAll) 
...
category_rma <- function(category, data, cat_type) {
  ...
  if (category=="" & cat_type == "") {
    rma_result <- rma(
      ...
      method = "REML",
      slab = AuthorY,
      control = list(stepadj = 1, maxiter = 1000)
    )
  } else {
...
# overall DA effects - remove duplicate participant studies: Weber, 2016 PIT; Westbrook 2024 PILT - these have the smaller effect size compared with Weber, 2016 DD and Westbrook, 2020 cognitive effort - ie this is the more conservative approach. 
datDA_final_overall_r <- datDA_final_cat %>% filter(Category != 'risk attitude', Category != 'model-based', Category != 'aversive pavlovian', Category != 'punishment learning sensitivity', !(AuthorY == 'Weber, 2016' & task_name == 'PIT'), !(AuthorY == 'Westbrook, 2024' & task_name == 'PILT WM')) %>% mutate(d = ifelse (Category == 'reward discounting', d * -1, d))
...
rma_simple_forest <- function(category,
  ...
  # ADD THE REFERENCE NUMBER AS SUPERSCRIPT 
  if (neuromodulator=='5HT') {
    categories_with_ref <- c("reward learning sensitivity", "punishment learning sensitivity", 
                             "aversive pavlovian", "appetitive pavlovian", 
                             "reward discounting", "reward vigour")
  } else if (neuromodulator=='DA') {
    categories_with_ref <- c("reward learning sensitivity", "punishment learning sensitivity", 
                             "aversive pavlovian", "reward discounting", "reward vigour")
  }
  
  include_ref_no <- category %in% categories_with_ref
  
  # Create author year + superscript for forestplots in the main manuscript
  if (savename == 'secondary_cat' | startsWith(savename, "aversive pavl w") | startsWith(savename, "mb w") | startsWith(savename, "risk w") | savename == 'drug_accord_orig_drugaction') {
    slab_label_authorY <-  rma_model$data$AuthorY
  } else {
    slab_label_authorY <- if (include_ref_no) {
      # mapply(function(author, ref) bquote(.(author) ^ .(ref)), rma_model$data$AuthorY, rma_model$data$ref_no, SIMPLIFY = FALSE)
      mapply(function(author, ref) as.expression(bquote(.(author)^.(ref))), rma_model$data$AuthorY, rma_model$data$ref_no)
    } else {
      rma_model$data$AuthorY  # Just the author name
    }
  }
  ...
  p <- forest(
    ...
    slab = (paste0(
      slab_label_authorY, " (", ifelse(grepl("^[A-Z]+$", drug_dose), drug_dose, capitalize_first_letter(drug_dose)), ")"
    )),
    pch = 19,    
    transf = transf,
    mar = c(0,0,0,0),
...
create_summary_table <- function(categories, include_task_name = FALSE, include_simplified_target = FALSE) {
  summary_table <- lapply(names(categories), function(category_name) {
    ...
    tibble(
      ...
      task_name2 = if (include_task_name && category_name %in% c("reward_learning", "punishment_learning", "model_based_learning", "reward discounting")) {
...
dopamine_categories <- list(
  ...
  "reward_discounting" = dataframes$datDA_cat %>% filter(Category == "reward discounting")
)
...
all_model_results <- lapply(moderators, function(moderator) {
  ...
  if (moderator == "simplified_drug_target2") {
    ...
    selected_filtered_dataframes <- filtered_dataframes[c("mod_DA_rew", "mod_DA_pun", "mod_DA_disc")]
...
DA_disc_drug <- rma(d, vard, mods = ~ drug_activity, data = datDA_final_cat %>% filter(Category == 'reward discounting'))
DA_disc_drug_dir <- rma(d, vard, mods = ~ drug_activity-1, data = datDA_final_cat %>% filter(Category == 'reward discounting'))
...
datDA_all <- bind_rows(
  datDA  %>% select(AuthorY, Category, drug_dose, N_drug, N_placebo, design_true, treat_as,task, task_name, task_name2,outcome_name, measure,age_total, female_total, single_or_multiple_drugAdministrations, secondary_category, drug_activity, simplified_drug_target2), 
  datAll_mb_w_DA %>% filter(Include_final == "N need data incl w") %>% select(AuthorY, Category, drug_dose, N_drug, N_placebo, design_true, treat_as,task, task_name, task_name2,outcome_name, measure,age_total, female_total, single_or_multiple_drugAdministrations, secondary_category, drug_activity, simplified_drug_target2),
  datAll_risk_w_DA %>% filter(Include_final == "N need data incl w") %>% select(AuthorY, Category, drug_dose, N_drug, N_placebo, design_true, treat_as,task, task_name, task_name2,outcome_name, measure,age_total, female_total, single_or_multiple_drugAdministrations, secondary_category, drug_activity, simplified_drug_target2)
) 
...
datDA_tbl <- datDA_all %>%
  select(AuthorY, Category, drug_dose, N_drug, N_placebo, design_true, treat_as, task_name,outcome_name,age_total, female_total, single_or_multiple_drugAdministrations) %>%
  mutate(
    Category = case_when(
    ...
    design_true = ifelse(design_true == 'w' & treat_as == 'bw',
                         paste0('w', '†'), # Add a dagger as a superscript
    ...
    age_total = round(as.numeric(age_total), 2),
  ) %>%
  rename(
    Study = AuthorY,
    `Drug, Dose` = drug_dose,
    `N placebo` = N_placebo,
    `N drug` = N_drug,
    `Original design` = design_true,
    `Final design` = treat_as,
    Task = task_name,
    Outcome = outcome_name,
    `Study Age` = age_total,
    `Study Female Proportion` = female_total,
    `Drug regimen` = single_or_multiple_drugAdministrations
  ) %>%
  mutate(across(c(`Study Age`, `Study Female Proportion`, `Drug regimen`), ~ ifelse(is.na(.), "NA", .))) %>%
  # Replace NA with "NA" in specific columns
  mutate(
    `Drug regimen` = case_when(
      `Drug regimen` == "single" ~ "1",  # If "single", replace with "1"
      `Drug regimen` != "single" ~ ">1", # If not "single", replace with ">1"
      TRUE ~ as.character(`Drug regimen`)  # Keep other values unchanged (if any)
    )) %>%
...
write.xlsx(datDA_tbl, "./results/dopamine_studies.xlsx")
...
dat5HT_all <- bind_rows(
  dat5HT %>%
    select(
      ...
      age_total,
      female_total, single_or_multiple_drugAdministrations, secondary_category, drug_activity, simplified_drug_target2
    ),
  datAll_orig_w %>%
...
dat5HT_tbl <- dat5HT_all %>%
 ...
 rename(
  ...
  mutate(across(c(`Study Age`, `Study Female Proportion`, `Drug regimen`), ~ ifelse(is.na(.), "NA", .))) %>% # Replace NA with "NA" in specific columns
  mutate(
    `Drug regimen` = case_when(
      `Drug regimen` == "single" ~ "1",  # If "single", replace with "1"
      `Drug regimen` != "single" ~ ">1", # If not "single", replace with ">1"
...
write.xlsx(dat5HT_tbl, "./results/serotonin_studies.xlsx")
